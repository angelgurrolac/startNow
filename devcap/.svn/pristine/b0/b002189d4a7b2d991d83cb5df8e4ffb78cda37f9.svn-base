    ...
    <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data', 'id' => 'form_empleados']]); ?>
            ...
            <div class="row form-container">  <!-- Form captura  -->
                ...
                <h3 class="campos-title font-bold">
                    <div class='circle-row izq'>3</div>Contactos
                </h3>
                <div class="clear"></div>

                <?= $form->field($modelContactos, 'NOMBRE_PADRE', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?> 

                <?= $form->field($modelContactos, 'TEL_PADRE', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?>

                <?= $form->field($modelContactos, 'NOMBRE_MADRE', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?>

                <?= $form->field($modelContactos, 'TEL_MADRE', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?>

                <?= $form->field($modelContactos, 'CASO_ACCIDENTE', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?>
                 <?= $form->field($modelContactos, 'TEL_ACCIDENTE', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?>
