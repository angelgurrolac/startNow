    ...
    <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data', 'id' => 'form_empleados']]); ?>
            ...
            <div class="row form-container">  <!-- Form captura  -->
                ...
                <h3 class="campos-title font-bold">
                    <div class='circle-row izq'>4</div>Beneficiario
                </h3>
                <div class="clear"></div>

                <?= $form->field($modelAdministradoraBenef, 'NOMBRE_BEN', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?> 

                <?= $form->field($modelAdministradoraBenef, 'RFC_BEN', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?>

                <?= $form->field($modelAdministradoraBenef, 'PARENTESCO_BEN', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?>

                <?= $form->field($modelAdministradoraBenef, 'PORCENTAJE', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?>

                <?= $form->field($modelAdministradoraBenef, 'DOMICILIO', 
                    [
                        'template' => ' <div class="'.$colums.'">{label}{input}{error}{hint}<div class="clear"></div></div>',
                    ]
                    )->textInput(['maxlength' => true]) ?>
...
<script>
    jQuery(document).ready(function(){
        ...
        $('#botonBeneficiario').on('click', function(){
         $( ".starthidden" ).show();
         $("#cantidadBeneficiarios").val("2");
});
