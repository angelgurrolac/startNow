 ...
 */
class CatalogorhController extends Controller
{
    ...
    public function actionIndex()
    {
        $dataProvider = new ActiveDataProvider([
            'query' => tblcatresponsablesrh::find(),
        ]);
    ...
    {
        $model = new tblcatresponsablesrh();

        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->PK_RESPONSABLE_RH]);
        } else {
    ...
    {
        if (($model = tblcatresponsablesrh::findOne($id)) !== null) {
            return $model;
        } else {
